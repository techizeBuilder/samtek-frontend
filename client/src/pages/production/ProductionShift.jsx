...
import { Clock, Package, Users, Eye, Save, Plus, Trash2, Calendar, Filter, RefreshCw } from 'lucide-react';
...
export default function ProductionShift() {
  ...
  useEffect(() => {
    console.log('🎯 ProductionShift component mounted or date changed, calling fetchProductionShiftData');
    fetchProductionShiftData(selectedDate);
  }, [selectedDate]);

  // Fetch production groups with shift data
  const fetchProductionShiftData = async (date) => {
    console.log('🚀 Starting fetchProductionShiftData for date:', date);
    setLoading(true);
    try {
      const url = date 
        ? `/api/production/production-shift?date=${date}` 
        : '/api/production/production-shift';
  ...
  const handleUngroupedBatchDataChange = async (itemKey, field, value) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        await fetchProductionShiftData(selectedDate);
      }
    } catch (error) {
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <Button 
            variant="outline" 
            size="icon"
            onClick={() => fetchProductionShiftData(selectedDate)}
            disabled={loading}
