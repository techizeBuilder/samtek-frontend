...
import { Check, X, Clock, Calendar, User, MapPin, Briefcase, IndianRupee, Loader2, Search } from 'lucide-react';
...
export default function TravelRequest() {
  ...
  // via `page`, see travelRequestController.js getManagerTravelRequests).
  ...
  const fetchRequests = async () => {
    try {
      ...
      const response = await api.get(`/travel-requests/manager?${params.toString()}`);
  ...
  const handleStatusUpdate = async (id, status) => {
    try {
      ...
      toast({
        title: "Success",
        description: `Travel request ${status.toLowerCase()} successfully`,
      });
      fetchRequests();
    } catch (error) {
      toast({
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center gap-4 flex-wrap">
        <div>
          <h1 className="text-3xl font-bold text-slate-800 tracking-tight">Travel Requests</h1>
          <p className="text-slate-500 mt-1">Review and approve team business travel plans.</p>
        </div>
              ...
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={6} className="text-center py-20">
                    <div className="flex flex-col items-center justify-center space-y-3">
                      ...
                      <p className="text-slate-500 font-medium">No travel requests found</p>
                    </div>
                  </TableCell>
                </TableRow>
              ) : (
                requests.map((request) => (
                  <TableRow key={request._id} className="hover:bg-slate-50/50 transition-colors">
                    <TableCell>
                      <div className="flex items-center gap-3">
                        <div className="h-9 w-9 rounded-full bg-cyan-100 flex items-center justify-center text-cyan-700 font-bold text-xs">
                    ...
                    <TableCell>
                      <div className="space-y-1">
                        <div className="flex items-center text-sm font-medium text-slate-700">
                          <MapPin className="h-3.5 w-3.5 mr-1.5 text-red-400" />
                          {request.destination}
                        </div>
                    ...
                    <TableCell>
                      <div className="flex items-center font-semibold text-slate-900 text-sm">
                        <IndianRupee className="h-3 w-3 mr-0.5 text-slate-500" />
