...
import { Package, CheckCircle2, Clock, Plus, X } from 'lucide-react';
...
function CreateJobModal({ order, onClose }) {
  ...
  const handle = async () => {
    ...
    try {
      ...
      toast({ title: 'Packaging job created', description: `Job created for ${order.orderId}` });
      onClose();
    } catch (e) {
    ...
    <div className="p-6 space-y-6 bg-slate-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">QC-approved production orders ready for packaging</p>
        </div>
      ...
      {readyLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map((order) => {
            ...
            return (
              <Card key={order._id} className="border-none shadow-sm hover:shadow-md transition-all duration-200">
                <CardContent className="p-5">
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <p className="font-semibold text-slate-800">{order.orderId}</p>
                      <p className="text-sm text-slate-500">{order.machineName}</p>
                    </div>
