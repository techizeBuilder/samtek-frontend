...
const StoreOrders = () => {
  ...
  const [productTypeLoading, setProductTypeLoading] = useState(null); // saleId of the one being saved
  const [localProductTypes, setLocalProductTypes] = useState({}); // saleId -> productType
  ...
  const storeOrders = orders.filter(item => {
    const hasGatePass = item.gatePass?.status === 'Generated';
    const matchesSearch =
      (item.orderCode || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
      (item.customerName || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
      (item.gatePass?.gatePassNumber || '').toLowerCase().includes(searchTerm.toLowerCase());
    return hasGatePass && matchesSearch;
  });
  ...
  const handleUpdateProductType = async (saleId, productType) => {
    if (!productType) return;
    setProductTypeLoading(saleId);
    try {
      await apiRequest('PATCH', `/api/orders/sale/${saleId}/product-type`, { productType });
      setLocalProductTypes(prev => ({ ...prev, [saleId]: productType }));
      toast({ title: "Saved", description: "Product type updated successfully" });
    } catch (error) {
      toast({ title: "Error", description: "Failed to update product type", variant: "destructive" });
    } finally {
      setProductTypeLoading(null);
    }
  };
          ...
          <Table>
            <TableHeader className="bg-slate-50">
              <TableRow>
                ...
                <TableHead className="w-[220px]">Product Type</TableHead>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow key={item._id} className="hover:bg-slate-50/50">
                  ...
                  <TableCell>
                    <Badge variant="outline" className="font-medium border-emerald-200 bg-emerald-50 text-emerald-700">
                      {item.gatePass?.gatePassNumber}
                    </Badge>
                    <div className="text-[10px] text-slate-400 mt-1">
                      {new Date(item.gatePass?.generatedAt).toLocaleDateString()}
                    </div>
                  </TableCell>
                  {/* Product Type Column */}
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <Select
                        value={localProductTypes[item._id] ?? (item.productType || '')}
                        onValueChange={(val) => handleUpdateProductType(item._id, val)}
                        disabled={productTypeLoading === item._id}
                      >
                      ...
                      {productTypeLoading === item._id && (
                        <RefreshCw className="w-3 h-3 animate-spin text-blue-500 shrink-0" />
                      )}
                      {(localProductTypes[item._id] || item.productType) && productTypeLoading !== item._id && (
